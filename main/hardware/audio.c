...
// #define CONFIG_EXAMPLE_PDM_TX_DAC 1
...
#define EXAMPLE_PDM_TX_FREQ_HZ          10000           // I2S PDM TX frequency 16k and higher you can hear digital glitch, 10k seems quite nice
...
void AUDIO_PlayTone(uint16_t freq, int16_t duration_ms)
{
    ...
    uint32_t duration_sine = (EXAMPLE_PDM_TX_FREQ_HZ / (float)freq) + 0.5;

    /* Generate the tone buffer */
    for (int i = 0; i < duration_sine; i++) {
        w_buf[i] = (int16_t)((sin(2 * (float)i * CONST_PI / duration_sine)) * EXAMPLE_WAVE_AMPLITUDE);
    }

    uint32_t duration_i2s = duration_ms * EXAMPLE_PDM_TX_FREQ_HZ / 1000;

    for (int tot_bytes = 0; tot_bytes < duration_i2s; tot_bytes += w_bytes) {
            /* Play the tone */
        if (i2s_channel_write(beeper_chan, w_buf, duration_sine * sizeof(int16_t), &w_bytes, 1000) != ESP_OK) {
...
{
    beeper_chan = i2s_example_init_pdm_tx();
    while(1){
        AUDIO_PlayTone(440, 500);
...
void i2s_example_pdm_tx_task(void *args)
{
    ...
    while (1) {
        int tone_point = EXAMPLE_SINE_WAVE_LEN(tone[tone_select][song[cnt] - 1]); //tone len
        ESP_LOGI(TAG, "tone point: %d", tone_point);
        /* Generate the tone buffer */
